...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Retrieve Checkout Session and PaymentIntent (handle both string/expanded cases)
    ...
    // If already captured, canceling the auth is not possible—return a clear error
    if (pi.status === "succeeded") {
      return res.status(400).json({ error: "Payment already captured; cannot cancel authorization" });
    }

    // Mark as issue reported / on hold before cancel (keeps metadata intact)
    const meta = { ...(pi.metadata || {}), fep_status: "issue_reported" };
